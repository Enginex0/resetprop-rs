...
| File | Purpose |
|------|---------|
| `crates/resetprop/tests/tier_b_child_smoke.rs` | Off-device integration test: fork a child owning a fake `__system_property_update`, install hook, seal one of two prop names, verify locked name's value bytes unchanged and free name's bytes updated via `process_vm_readv` |
...
3. **Task 3**: Implement the remote installer `install_trampoline(handle: &mut HookHandle) -> Result<()>` — (a) call `build_hook_body_bytes(lock_list_vaddr, saved_prologue_vaddr, return_addr)` and write the returned `Vec<u8>` into `handle.hook_page + HOOK_BODY_OFFSET` (HOOK_BODY_OFFSET = 4 inside the lock-list-first layout: byte 0 is the empty-list sentinel NUL) via `process_vm_writev`; (b) write 16-byte trampoline `[LDR_X16_PC8, BR_X16, target_lo, target_hi]` at `handle.target_fn` (overwriting `handle.saved_prologue`) via `process_vm_writev`; (c) i-cache sync — primary: remote `membarrier(MEMBARRIER_CMD_PRIVATE_EXPEDITED_SYNC_CORE=0x80, flags=0, cpu_id=0)` via `__NR_membarrier = 283`; fallback on `EINVAL`/`EPERM`: single `isb` (`0xd5033fdf`) staged into a scratch slot in the hook page and executed via ptrace register flip (`pc = hook_page + isb_slot`, then `svc`-staged resume); returns `Error::SealHookError` on all write failures. The installer contains no opcode encoding of its own — the deterministic byte layout lives entirely in `build_hook_body_bytes` — Files: `crates/resetprop/src/seal/hook.rs` — Verifies: `cargo test -p resetprop --test tier_b_child_smoke -- --ignored --test-threads=1` reaches the assertion block without `SealHookError`
...
4. **Hook-page layout constants.** `LOCK_LIST_OFFSET = 0`, `HOOK_BODY_OFFSET = 4` — byte 0 is the initial empty-list sentinel NUL (written by P03 when the page was allocated), bytes 1..=3 are zero-padding to 4-byte alignment, hook body starts at byte 4. The list grows downward from offset 0 toward the hook body; P03 reserved the first 1024 bytes of the 4 KB hook page for the list, leaving 3072 bytes for the ≤176-byte hook body plus strcmp splice and RESTORE/LOCK_LIST literals.
...
cargo test -p resetprop --test tier_b_child_smoke -- --ignored --test-threads=1   # integration test: locked.prop bytes unchanged, free.prop bytes updated
cargo test -p resetprop                                                     # full regression over P01..P03 modules passes
```

Expected output of the integration test:

- `locked.prop: hook must prevent child from mutating value bytes` — PASS
- `free.prop: non-sealed name must continue updating` — PASS
