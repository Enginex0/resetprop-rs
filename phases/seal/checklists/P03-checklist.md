...
- [ ] **Correctness** — Edge cases walked through (list them): zero-phnum ELF, PT_DYNAMIC absent, PT_DYNAMIC entry count derived from `p_filesz / 16`, DT_STRSZ optional, vaddr outside every PT_LOAD range: ___________________________
...
- [ ] **Correctness** — Edge cases: `nbuckets == 0`, `buckets[h % nbuckets] == 0`, chain index underflow when `n < symoffset`, bloom `bloom_size == 0`, symbol name unterminated within `strtab_size`: ___________________________
